...
void CollisionDetectionSAT::genContactPoints(Manifold * out_manifold)
{
	...
	if (polygon1.size() == 0 || polygon2.size() == 0) {
		...
		bool flipped = fabs(Vector3::dot(bestColData.normal, normal1)) < fabs(Vector3::dot(bestColData.normal, normal2));
		...
		for (const Vector3& point : polygon2) {
			...
			float contact_penetration = Vector3::dot(pointDiff, bestColData.normal);

			Vector3 globalOnA = point;
			Vector3 globalOnB = point - bestColData.normal * contact_penetration;
